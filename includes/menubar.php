...
?>
		<nav class="navbar navbar-expand navbar-light navbar-bg">
				...
				if(isset($_POST['programs']))
					...
					$programs=$_SESSION['program'];
				?>
				<?php if(count($arprograms)>1){?>
				<form name="frmchangeprogram" id="frmchangeprogram" class="d-none  d-sm-inline-block" method="post" action="/oceanerp/<?php echo $_SESSION['page'];?>" >
				<div class="input-group input-group-navbar">
                	<select id="programs" name="programs" class="form-control" onchange="SamesetFormAction('/oceanerp/<?php echo $_SESSION['page'];?>')" >
                      <option value="">Select Program</option>
                    	<?php
						   	foreach($arprograms as $program){
							$programrec=$func_handler->getColumnData("tblprogram","collegename" ,"program", $program);
                            ?>
                              <option  <?php if ($programs== $program or $arprograms[0]==$program ) echo 'selected' ; ?>  value='<?php echo $program;?>'><?php echo $programrec['collegename'];?></option>
                          <?php
                          }
                        ?>
                	</select> 
				...
				<?php }else{?>
				<form name="frmchangeprogram" id="frmchangeprogram" class="d-none  d-sm-inline-block" method="post" action="/oceanerp/home.php" >
				<div class="input-group input-group-navbar">
                	<select id="programs" name="programs" class="form-control" onchange="SamesetFormAction('/oceanerp/home.php')" >
                   	<?php
						   	foreach($arprograms as $program){
							$programrec=$func_handler->getColumnData("tblprogram","collegename" ,"program", $program);
                            ?>
                              <option  <?php if ($programs== $program ) echo 'selected' ; ?>  value='<?php echo $program;?>'><?php echo $programrec['collegename'];?></option>
                          <?php
                          }
                        ?>
                	</select> 
					...
					<ul class="navbar-nav navbar-align">
						...
						<li class="nav-item dropdown">
							<a class="nav-icon pe-md-0 dropdown-toggle" href="#" data-bs-toggle="dropdown">
								<img src="<?php echo $authuser[0]['picture'];?>" class="avatar img-fluid rounded" alt="<?php echo ucwords($authuser[0]['first_name'])." ".ucwords($authuser[0]['last_name']);?>" />
							</a>
							<div class="dropdown-menu dropdown-menu-end">
								...
								<div class="dropdown-divider"></div>
								<a class="dropdown-item" href="/oceanerp/logout.php">ERP Log out</a>
								<a class="dropdown-item" href="/oceanerp/logoutg.php">Google Sign out</a>
							</div>
						</li>
					</ul>
				</div>
